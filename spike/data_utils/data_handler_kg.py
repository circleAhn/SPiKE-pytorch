...
class DataHandlerKG:
    def __init__(self) -> None:
        ...
        if configs['model']['name'] == 'spike':
            user_npz = np.load(path.join(predir, 'user_embeddings.npz'))
            user_ids = sorted(range(len(user_npz['ids'])), key=lambda i: int(user_npz['ids'][i]))
            self.text_user_emb = torch.from_numpy(user_npz['embeddings'][user_ids]).to(configs['device'])

            item_npz = np.load(path.join(predir, 'item_embeddings.npz'))
            item_ids = sorted(range(len(item_npz['ids'])), key=lambda i: int(item_npz['ids'][i]))
            entity_npz = np.load(path.join(predir, 'entity_embeddings.npz'))
            entity_ids = sorted(range(len(entity_npz['ids'])), key=lambda i: int(entity_npz['ids'][i]))
            self.text_entity_emb = torch.from_numpy(np.concatenate([item_npz['embeddings'][item_ids], entity_npz['embeddings'][entity_ids]], axis=0)).to(configs['device'])
        ...
        if configs['model']['name'] == 'spike':
            ...
            item_npz = np.load(path.join(predir, 'item_embeddings.npz'))
            item_ids = sorted(range(len(item_npz['ids'])), key=lambda i: int(item_npz['ids'][i]))
            self.text_item_emb = torch.from_numpy(item_npz['embeddings'][item_ids]).to(configs['device'])
